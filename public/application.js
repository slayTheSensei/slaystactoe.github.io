...
var onSignUp = function onSignUp(event) {
  ...
  var data = getFormFields(event.target); // This is targeting the data i want to send
...
var onCreateGame = function onCreateGame(event) {
  event.preventDefault();
  api.createGame().then(ui.gamePostSuccess).catch(ui.fail);
};

// event handler to update game states
var updateGame = function updateGame(index, value, over) {
  api.updateMoves(index, value, over).then(ui.onUpdateSuccess).catch(ui.onUpdateFail);
};

var onGetGames = function onGetGames(event) {
  event.preventDefault();
  // console.log('here')
  api.retrieveGames().then(ui.getGameSuccess).catch(ui.fail);
};

var resetGame = function resetGame() {
  api.createGame().then(ui.onResetGameSuccess).catch(ui.onResetGameFail);
};
...
var gamePostSuccess = function gamePostSuccess(data) {
  $('#grid').show();
  app.game = data.game;
...
var onResetGameSuccess = function onResetGameSuccess(data) {
  // app.game = data.game
  // app.game.id = data.game.id
...
module.exports = {
  ...
  gamePostSuccess: gamePostSuccess,
